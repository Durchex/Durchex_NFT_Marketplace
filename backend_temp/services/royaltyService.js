...
// RoyaltyService: Manages royalty configuration, tracking, and distribution
class RoyaltyService {
	...
	async getRoyaltyInfo(collection, tokenId, salePrice) {
		try {
			...
			const [recipients, amounts] = await this.contract.getRoyaltyInfo(collection, tokenId, salePrice);
			const result = {
				recipients,
	...
	async calculateBatchRoyalties(sales) {
		try {
			...
			for (const sale of sales) {
				const royaltyInfo = await this.getRoyaltyInfo(sale.collection, sale.tokenId, sale.salePrice);
				results.push({
					...sale,
					royaltyInfo,
				});
